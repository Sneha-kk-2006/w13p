  ...
  try {
    ...
    if (
      ...
      return res
        ...
        .json({
          success: false,
          message:
            "A delivered order cannot be set back to pending or shipped.",
        });
    }
    if (order.orderStatus === "Cancelled" && status !== "Cancelled") {
      return res
        ...
        .json({
          success: false,
          message: "A cancelled order cannot be changed to another status.",
        });
    }
    if (order.orderStatus === "Returned" && status !== "Returned") {
      return res
        ...
        .json({
          success: false,
          message: "A returned order cannot be changed to another status.",
        });
    }
        ...
        if (isCancelling) {
          ...
          await walletService.credit(
            ...
            `Refund for returned order ${order.orderId}`,
            {
              ...
              idempotencyKey: `return_full_${order._id}`,
            },
          );
        }
      }
    ...
    if (isNewCancellation || isNewReturn) {
      ...
      if (status === "Cancelled") {
        ...
        const refundAmount = parseFloat(
          (itemTotalPrice * (1 - discountRatio)).toFixed(2),
        );
